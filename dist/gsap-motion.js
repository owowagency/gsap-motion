...
function no(S, p, m) {
  ...
  function je(F) {
    var Se = K, lt = N;
    return K = N = void 0, ge = F, re = S.apply(lt, Se), re;
  }
  function Rt(F) {
  ...
  function Ut(F) {
    return C = void 0, Ue && K ? je(F) : (K = N = void 0, re);
  }
  function xt() {
    C !== void 0 && clearTimeout(C), ge = 0, K = H = N = C = void 0;
  }
  function ht() {
...
var so = no;
const lo = /* @__PURE__ */ Ni(so), Ln = class {
  /**
   * @example
...
class ao {
  constructor(p) {
    ...
    q(this, "observable");
    const [m, K] = [p].flat();
    this.element = typeof m == "string" ? document.querySelector(m) : m, this.axis = K, this.observable = new Hi((N) => {
      const w = new ResizeObserver(
        (re) => this.handleResize(re, N)
      );
  ...
  handleResize(p, m) {
    ...
    if (!K)
      return;
    const { inlineSize: N, blockSize: w } = K.borderBoxSize[0], re = N !== this.inlineSize, C = w !== this.blockSize, H = this.inlineSize == null || this.blockSize == null;
    if (this.inlineSize = N, this.blockSize = w, !H) {
    ...
    */
    var w, re, C, H, ge, Ae, pt, Ue, je, Rt, Je, Re, Ot, Ut = function() {
      ...
      var r = e, i = e, a = Se(), s = a, d = n || 50, T = Math.max(500, d * 3), Y = function(_, $) {
        ...
        var $ = s, ie = i, W = Se();
        return (_ || _ === 0) && _ !== r && Y(_), a === s || W - s > T ? 0 : (r + (t ? ie : -ie)) / ((t ? W : a) - $) * 1e3;
      };
      return {
        update: Y,
        reset: X,
    ...
    var ae = function() {
      ...
      return e.init = function(t) {
        ...
        var r = t.tolerance, i = t.dragMinimum, a = t.type, s = t.target, d = t.lineHeight, T = t.debounce, Y = t.preventDefault, X = t.onStop, g = t.onStopDelay, u = t.ignore, _ = t.wheelSpeed, $ = t.event, ie = t.onDragStart, W = t.onDragEnd, Ke = t.onDrag, te = t.onPress, I = t.onRelease, Pe = t.onRight, oe = t.onLeft, E = t.onUp, Ze = t.onDown, mt = t.onChangeX, M = t.onChangeY, Ce = t.onChange, k = t.onToggleX, zt = t.onToggleY, Ee = t.onHover, Xe = t.onHoverEnd, Be = t.onMove, G = t.ignoreCheck, pe = t.isNormalizer, ce = t.onGestureStart, l = t.onGestureEnd, he = t.onWheel, nr = t.onEnable, Nt = t.onDisable, ut = t.onClick, Wt = t.scrollSpeed, U = t.capture, De = t.allowClicks, He = t.lockAxis, Dr = t.onLockAxis;
        this.target = s = Qe(s) || ge, this.vars = t, u && (u = w.utils.toArray(u)), r = r || 1e-9, i = i || 0, _ = _ || 1, Wt = Wt || 1, a = a || "wheel,touch,pointer", T = T !== !1, d || (d = parseFloat(C.getComputedStyle(Ae).lineHeight) || 22);
        var nt, ft, L, me, it, kt, Ne, c = this, At = 0, Q = 0, Gt = Xt(s, Ie), $t = Xt(s, de), gr = Gt(), We = $t(), zr = ~a.indexOf("touch") && !~a.indexOf("pointer") && Re[0] === "pointerdown", qt = Yt(s), se = s.ownerDocument || H, ot = [0, 0, 0], Ge = [0, 0, 0], Ar = 0, $e = function() {
          return Ar = Se();
        }, Ct = function(y, f) {
          ...
          He && !Ne && (c.axis = Ne = Math.abs(y) > Math.abs(f) ? "x" : "y", kt = !0), Ne !== "y" && (ot[2] += y, c._vx.update(y, !0)), Ne !== "x" && (Ge[2] += f, c._vy.update(f, !0)), T ? nt || (nt = requestAnimationFrame(Vt)) : Vt();
        }, or = function(y) {
          if (!Ct(y, 1)) {
            y = wr(y, Y);
          ...
          Ct(R, 1) || R && R.button || (c.axis = Ne = null, ft.pause(), c.isPressed = !0, R = wr(R), At = Q = 0, c.startX = c.x = R.clientX, c.startY = c.y = R.clientY, c._vx.reset(), c._vy.reset(), Te(pe ? s : se, Re[1], or, Y, !0), c.deltaX = c.deltaY = 0, te && te(c));
          ...
          if (!Ct(R, 1)) {
            ...
            })), c.isDragging = c.isGesturing = c.isPressed = !1, X && !pe && ft.restart(!0), W && f && W(c), I && I(c, f);
          }
        }, bt = function(y) {
          ...
          if (!Ct(y)) {
            var f = Gt(), v = $t();
            _r((f - gr) * Wt, (v - We) * Wt, 1), gr = f, We = v, X && ft.restart(!0);
          }
        }, yt = function(y) {
          if (!Ct(y)) {
            y = wr(y, Y), he && (it = !0);
        ...
        ft = c._dc = w.delayedCall(g || 0.25, Ir).pause(), c.deltaX = c.deltaY = 0, c._vx = un(0, 50, !0), c._vy = un(0, 50, !0), c.scrollX = Gt, c.scrollY = $t, c.isDragging = c.isGesturing = c.isPressed = !1, Ot(this), c.enable = function(R) {
          ...
          }).length || V(qt ? se : s, "scroll", cn), c.isPressed && (c._vx.reset(), c._vy.reset(), V(pe ? s : se, Re[1], or, !0)), V(qt ? se : s, "scroll", dt, U), V(s, "wheel", yt, U), V(s, Re[0], z, U), V(se, Re[2], It), V(se, Re[3], It), V(s, "click", $e, !0), V(s, "click", Lr), V(se, "gesturestart", bt), V(se, "gestureend", wt), V(s, Ue + "enter", vr), V(s, Ue + "leave", Lt), V(s, Ue + "move", sr), c.isEnabled = c.isPressed = c.isDragging = !1, Nt && Nt(c));
     ...
     * @author: Jack Doyle, jack@greensock.com
    */
    var h, ar, A, j, gt, ee, Hn, Nr, Wr, cr, Gr, $r, Oe, qr, fn, Le, Nn, Wn, ur, Gn, dn, $n, et, qn, Vn, Un, Bt, pn, hn, gn, Vr = 1, Fe = Date.now, _n = Fe(), at = 0, yr = 0, Si = function o() {
      ...
      var T = i.startColor, Y = i.endColor, X = i.fontSize, g = i.indent, u = i.fontWeight, _ = j.createElement("div"), $ = Qt(t) || St(t, "pinType") === "fixed", ie = e.indexOf("scroller") !== -1, W = $ ? ee : t, Ke = e.indexOf("start") !== -1, te = Ke ? T : Y, I = "border-color:" + te + ";font-size:" + X + ";color:" + te + ";font-weight:" + u + ";pointer-events:none;white-space:nowrap;font-family:sans-serif,Arial;z-index:1000;padding:4px 8px;border-width:0;border-style:solid;";
      return I += "position:" + ((ie || d) && $ ? "fixed;" : "absolute;"), (ie || d || !$) && (I += (r === de ? mn : bn) + ":" + (a + parseFloat(g)) + "px;"), s && (I += "box-sizing:border-box;text-align:left;width:" + s.offsetWidth + "px;"), _._isStart = Ke, _.setAttribute("class", "gsap-marker-" + e + (n ? " marker-" + n : "")), _.style.cssText = I, _.innerText = n || n === 0 ? e + "-" + n : e, W.children[0] ? W.insertBefore(_, W.children[0]) : W.appendChild(_), _._offset = _["offset" + r.op.d2], rn(_, 0, r, Ke), _;
    }, rn = function(e, n, t, r) {
      var i = {
        display: "block"
      ...
      b.cache++, !Oe && !$n && !j.fullscreenElement && !j.webkitFullscreenElement && (!qn || Un !== A.innerWidth || Math.abs(A.innerHeight - Vn) > A.innerHeight * 0.25) && Nr.restart(!0);
    }, Jt = {}, Ri = [], si = function o() {
      ...
      }), ai(hn, 1), Nr.pause(), tr++, rt = 2, Dt(2), P.forEach(function(r) {
      ...
      var _ = u ? u.time() : 0, $, ie, W;
      if (u && u.seek(0), Sr(e))
        u && (e = h.utils.mapRange(u.scrollTrigger.start, u.scrollTrigger.end, 0, g, e)), s && rn(s, t, r, !0);
      else {
        Ye(n) && (n = n(d));
        var Ke = (e || "0").split(" "), te, I, Pe, oe;
        W = Qe(n) || ee, te = Pt(W) || {}, (!te || !te.left && !te.top) && vt(W).display === "none" && (oe = W.style.display, W.style.display = "block", te = Pt(W), oe ? W.style.display = oe : W.style.removeProperty("display")), I = en(Ke[0], te[r.d]), Pe = en(Ke[1] || "0", t), e = te[r.p] - T[r.p] - Y + I + i - Pe, s && rn(s, Pe, r, t - Pe < 20 || s._isStart && Pe > 20), t -= t - Pe;
      }
      if (a) {
      ...
      return u && W && ($ = Pt(W), u.seek(g), ie = Pt(W), u._caScrollDist = $[r.p] - ie[r.p], e = e / u._caScrollDist * g), u && u.seek(_), u ? e : Math.round(e);
      ...
      return e.init = function(t, r) {
        ...
        var i = t, a = i.onUpdate, s = i.toggleClass, d = i.id, T = i.onToggle, Y = i.onRefresh, X = i.scrub, g = i.trigger, u = i.pin, _ = i.pinSpacing, $ = i.invalidateOnRefresh, ie = i.anticipatePin, W = i.onScrubComplete, Ke = i.onSnapComplete, te = i.once, I = i.snap, Pe = i.pinReparent, oe = i.pinSpacer, E = i.containerAnimation, Ze = i.fastScrollEnd, mt = i.preventOverlaps, M = t.horizontal || t.containerAnimation && t.horizontal !== !1 ? Ie : de, Ce = !X && X !== 0, k = Qe(t.scroller || A), zt = h.core.getCache(k), Ee = Qt(k), Xe = ("pinType" in t ? t.pinType : St(k, "pinType") || Ee && "fixed") === "fixed", Be = [t.onEnter, t.onLeave, t.onEnterBack, t.onLeaveBack], G = Ce && t.toggleActions.split(" "), pe = "markers" in t ? t.markers : Zr.markers, ce = Ee ? 0 : parseFloat(vt(k)["border" + M.p2 + dr]) || 0, l = this, he = t.onRefreshInit && function() {
          return t.onRefreshInit(l);
        }, nr = Ti(k, Ee, M), Nt = ki(k, Ee), ut = 0, Wt = 0, U = Xt(k, M), De, He, Dr, nt, ft, L, me, it, kt, Ne, c, At, Q, Gt, $t, gr, We, zr, qt, se, ot, Ge, Ar, $e, Ct, Ir, Vt, _r, ir, or, z, It, bt, wt, dt, yt, sr, vr, Lt;
        if (pn(l), l._dir = M, ie *= 45, l.scroller = k, l.scroll = E ? E.time.bind(E) : U, nt = U(), l.vars = t, r = r || t.animation, "refreshPriority" in t && (Gn = 1, t.refreshPriority === -9999 && (Pr = l)), zt.tweenScroll = zt.tweenScroll || {
          ...
          targets: g,
          className: s
        }), u && (_ === !1 || _ === ct || (_ = !_ && u.parentNode && u.parentNode.style && vt(u.parentNode).display === "flex" ? !1 : ne), l.pin = u, He = h.core.getCache(u), He.spacer ? Gt = He.pinState : (oe && (oe = Qe(oe), oe && !oe.nodeType && (oe = oe.current || oe.nativeElement), He.spacerIsNative = !!oe, oe && (He.spacerState = sn(oe))), He.spacer = We = oe || j.createElement("div"), We.classList.add("pin-spacer"), d && We.classList.add("pin-spacer-" + d), He.pinState = Gt = sn(u)), t.force3D !== !1 && h.set(u, {
          force3D: !0
        }), l.spacer = We = He.spacer, _r = vt(u), Ar = _r[_ + M.os2], qt = h.getProperty(u), se = h.quickSetter(u, M.a, ke), Mn(u, We, _r), gr = sn(u)), pe) {
          At = jr(pe) ? ri(pe, ii) : ii, Ne = tn("scroller-start", d, k, M, At, 0), c = tn("scroller-end", d, k, M, At, 0, Ne), zr = Ne["offset" + M.op.d2];
          var Lr = Qe(St(k, "content") || k);
          it = this.markerStart = tn("start", d, Lr, M, At, zr, 0, E), kt = this.markerEnd = tn("end", d, Lr, M, At, zr, 0, E), E && (vr = h.quickSetter([it, kt], M.a, ke)), !Xe && !(F.length && St(k, "fixedMarkers") === !0) && (Ci(Ee ? ee : k), h.set([Ne, c], {
            force3D: !0
          }), Ct = h.quickSetter(Ne, M.a, ke), Vt = h.quickSetter(c, M.a, ke));
        }
        if (E) {
        ...
        l.previous = function() {
          ...
          x !== l.isReverted && (x && (yt = Math.max(U(), l.scroll.rec || 0), dt = l.progress, sr = r && r.progress()), it && [it, kt, Ne, c].forEach(function(be) {
            return be.style.display = x ? "none" : "block";
          }), x && (Oe = l, l.update(x)), u && (!Pe || !l.isActive) && (x ? Pi(u, We, Gt) : Mn(u, We, vt(u), $e)), x || l.update(x), Oe = O, l.isReverted = x);
          ...
          if (!((Oe || !l.enabled) && !v)) {
            ...
            !rt && he && he(l), Oe = l, Wt = Fe(), De.tween && (De.tween.kill(), De.tween = 0), z && z.pause(), $ && r && r.revert({
              kill: !1
            }).invalidate(), l.isReverted || l.revert(!0, !0), l._subPinOffset = !1;
            for (var x = nr(), O = Nt(), be = E ? E.duration() : Ht(k, M), B = Q <= 0.01, Z = 0, Me = 0, we = t.end, ue = t.endTrigger || g, J = t.start || (t.start === 0 || !g ? 0 : u ? "0 0" : "0 100%"), st = l.pinnedContainer = t.pinnedContainer && Qe(t.pinnedContainer), qe = g && Math.max(0, P.indexOf(l)) || 0, ye = qe, le, ze, mr, lr, fe, xe, Et, On, mi, Fr, Mt; ye--; )
            ...
            if (L += Z, me += Z, B && (dt = h.utils.clamp(0, 1, h.utils.normalize(L, me, yt))), l._pinPush = Me, it && Z && (le = {}, le[M.a] = "+=" + Z, st && (le[M.p] = "-=" + U()), h.set([it, kt], le)), u)
              ...
              }, Mt.style["overflow" + M.a.toUpperCase()] = "scroll"), Mn(u, We, le), gr = sn(u), ze = Pt(u, !0), On = Xe && Xt(k, lr ? Ie : de)(), _ && ($e = [_ + M.os2, Q + Me + ke], $e.t = We, ye = _ === ne ? yn(u, M) + Q + Me : 0, ye && $e.push(M.d, ye + ke), hr($e), st && P.forEach(function(Yr) {
            ...
            }), l.start = L, l.end = me, nt = ft = rt ? yt : U(), !E && !rt && (nt < yt && U(yt), l.scroll.rec = 0), l.revert(!1, !0), wt && (ut = -1, l.isActive && U(L + Q * dt), wt.restart(!0)), Oe = 0, r && Ce && (r._initted || sr) && r.progress() !== sr && r.progress(sr, !0).render(r.time(), !0, !0), (B || dt !== l.progress || E) && (r && !Ce && r.totalProgress(E && L < -1e-3 && !dt ? h.utils.normalize(L, me, 0) : dt, !0), l.progress = (nt - L) / Q === dt ? 0 : dt), u && _ && (We._pinOffset = Math.round(l.progress * Ge)), z && z.invalidate(), Y && !rt && Y(l);
          }
          ...
          if (!(E && !x && !f)) {
            ...
            if (v && (ft = nt, nt = E ? U() : O, I && (or = ir, ir = r && !Ce ? r.totalProgress() : B)), ie && !B && u && !Oe && !Vr && at && L < O + (O - ft) / (Fe() - Gr) * ie && (B = 1e-4), B !== Z && l.enabled) {
              ...
              I && !De.tween && !Oe && !Vr && wt.restart(!0), s && (qe || te && B && (B < 1 || !gn)) && Wr(s.targets).forEach(function(fe) {
          ...
          x >= 0 && P.splice(x, 1), x === Le && nn > 0 && Le--, x = 0, P.forEach(function(O) {
            ...
            kill: !1
          }), v || r.kill()), it && [it, kt, Ne, c].forEach(function(O) {
        ...
        }), ve(A, "wheel", pr), ve(j, "scroll", pr), clearInterval($r), ve(j, "touchcancel", Tt), ve(ee, "touchstart", Tt), Qr(ve, j, "pointerdown,touchstart,mousedown", jn), Qr(ve, j, "pointerup,touchend,mouseup", Qn), Nr.kill(), Ur(ve);
        for (var i = 0; i < b.length; i += 3)
          Kr(ve, b[i], b[i + 1]), Kr(ve, b[i], b[i + 2]);
      }, o.enable = function() {
        if (A = window, j = document, gt = j.documentElement, ee = j.body, h && (Wr = h.utils.toArray, cr = h.utils.clamp, pn = h.core.context || Tt, dn = h.core.suppressOverwrites || Tt, hn = A.history.scrollRestoration || "auto", Cn = A.pageYOffset, h.core.globals("ScrollTrigger", o), ee)) {
          ...
          for (i.revert || Object.defineProperty(i, "revert", {
            ...
            return Vr = 0;
          }), _e(j, "touchcancel", Tt), _e(ee, "touchstart", Tt), Qr(_e, j, "pointerdown,touchstart,mousedown", jn), Qr(_e, j, "pointerup,touchend,mouseup", Qn), fn = h.utils.checkPrefix("transform"), on.push(fn), ar = Fe(), Nr = h.delayedCall(0.2, rr).pause(), ur = [j, "visibilitychange", function() {
            var d = A.innerWidth, T = A.innerHeight;
            j.hidden ? (Nn = d, Wn = T) : (Nn !== d || Wn !== T) && Or();
    ...
    D.version = "3.11.5", D.saveStyles = function(o) {
      return o ? Wr(o).forEach(function(e) {
        if (e && e.style) {
          var n = tt.indexOf(e);
    ...
    var hi = function(e, n, t, r) {
      ...
      return g && h.set(g, {
        ...
        k._ts && k.pause(), Fe() - W > 1e3 && (te = 0, W = Fe());
      }, e.onChange = function(G, pe, ce, l, he) {
        if (tr !== te && mt(), pe && t && _(E(l[2] === pe ? zt + (G.startX - G.x) : _() + pe - l[1])), ce) {
          u.offset && M();
          var nr = he[2] === ce, Nt = nr ? Ee + G.startY - G.y : u() + ce - he[1], ut = Ze(Nt);
          nr && Nt !== ut && (Ee += ut - Nt), u(ut);
        }
        (ce || pe) && Dt();
      }, e.onEnable = function() {
    ...
    super(
      (N) => {
        ...
        Object.assign(N.meta, {
          ...
          direction: "ltr",
          ...m
        }), N.meta.scrollTrigger = yi.ScrollTrigger.create({});
        let w = null;
        ...
        N.meta.sourceDOM = w.cloneNode(!0), N.meta.target = w, H.append(...w.childNodes), C.append(H), w == null || w.append(C), Ve.set(H, { display: "inline-flex" });
        ...
        const St = Ve.ticker.add(() => {
          var V, jt;
          switch (ht = N.meta.velocity != null ? N.meta.scrollTrigger.getVelocity() ?? 0 : 0, b = ht * N.meta.velocity, N.meta.direction) {
            case "ltr":
              xt = -1, b = -Math.abs(b);
              break;
            case "rtl":
              ...
              break;
            case "scroll":
              xt = N.meta.scrollTrigger.direction ?? 1;
              break;
            case "scroll-reverse":
              xt = -(N.meta.scrollTrigger.direction ?? 1), b = -b;
          }
          F = Ve.getProperty(C, "x"), Se = N.meta.speed * -xt, lt = (Se - b) * Ve.ticker.deltaRatio(), Ft = Ot(F + lt), Re(Ft), (jt = (V = N.meta).onUpdate) == null || jt.call(V, Ut(Ft));
        });
        return (Te = (Yt = N.meta).onCreated) == null || Te.call(Yt), () => {
          var V;
          for (Je.kill(!0), Ve.ticker.remove(St), w == null || w.replaceChildren(...N.meta.sourceDOM.childNodes); je.length; )

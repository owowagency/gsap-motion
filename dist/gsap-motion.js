...
function no(y, h, _) {
  ...
  function Ue(B) {
    var Ee = B - le, lt = B - de, ir = h - Ee;
    return st ? ro(ir, M - lt) : ir;
  }
  function Z(B) {
    var Ee = B - le, lt = B - de;
  ...
  return T.cancel = vr, T.flush = St, T;
}
function zn(y) {
  var h = typeof y;
...
let _r = Fn;
X(_r, "resetDebounceTime", 100), /** Target framerate */
X(_r, "referenceFramerate", 60);
class ao {
  constructor(h) {
    X(this, "axis");
    X(this, "element");
...
(function(y, h) {
  (function(_, G) {
    ...
    */
    var M, se, F, le, de, qe, st, Ze, pt, nr, Ue, Z, Rt, ht = function() {
      return M || typeof window < "u" && (M = window.gsap) && M.registerPlugin && M;
    }, vr = 1, St = [], T = [], B = [], Ee = Date.now, lt = function(e, n) {
      return n;
    }, ir = function() {
      ...
      var t = function r(i) {
        if (i || i === 0) {
          vr && (F.history.scrollRestoration = "manual");
      ...
      pt = M.core.globals().ScrollTrigger, pt && pt.core && ir();
    }, Bn = function(e) {
      return M = e || ht(), M && typeof document < "u" && document.body && (F = window, le = document, de = le.documentElement, qe = le.body, nr = [F, le, de, qe], M.utils.clamp, Rt = M.core.context || function() {
    ...
    var ae = function() {
      ...
      return e.init = function(t) {
        ...
        var rt, ft, I, be, nt, kt, Be, c = this, At = 0, V = 0, Ht = Ft(s, Ae), Wt = Ft(s, pe), dr = Ht(), He = Wt(), Or = ~a.indexOf("touch") && !~a.indexOf("pointer") && Z[0] === "pointerdown", Nt = br(s), ie = s.ownerDocument || le, it = [0, 0, 0], We = [0, 0, 0], Ar = 0, Ne = function() {
          return Ar = Ee();
        }, Ct = function(m, f) {
          ...
          if (!Ct(m, 1)) {
            ...
            c.x = f, c.y = b, (me || Math.abs(c.startX - f) >= i || Math.abs(c.startY - b) >= i) && (Qe && (I = !0), me || (c.isDragging = !0), Jt(w, P), me || re && re(c));
          }
        }, O = c.onPress = function(E) {
          Ct(E, 1) || E && E.button || (c.axis = Be = null, ft.pause(), c.isPressed = !0, E = mr(E), At = V = 0, c.startX = c.x = E.clientX, c.startY = c.y = E.clientY, c._vx.reset(), c._vy.reset(), Ve(he ? s : ie, Z[1], er, L, !0), c.deltaX = c.deltaY = 0, ee && ee(c));
          ...
          if (!Ct(m)) {
            var f = Ht(), b = Wt();
            pr((f - dr) * Bt, (b - He) * Bt, 1), dr = f, He = b, Y && ft.restart(!0);
          }
        }, xt = function(m) {
          if (!Ct(m)) {
            m = mr(m, L), ge && (nt = !0);
            var f = (m.deltaMode === 1 ? d : m.deltaMode === 2 ? F.innerHeight : 1) * v;
            pr(m.deltaX * f, m.deltaY * f, 0), Y && !he && ft.restart(!0);
          }
        }, tr = function(m) {
          if (!Ct(m)) {
            var f = m.clientX, b = m.clientY, w = f - c.x, P = b - c.y;
            c.x = f, c.y = b, be = !0, (w || P) && Jt(w, P);
          }
        }, hr = function(m) {
          c.event = m, ke(c);
        }, It = function(m) {
        ...
        ft = c._dc = M.delayedCall(g || 0.25, zr).pause(), c.deltaX = c.deltaY = 0, c._vx = un(0, 50, !0), c._vy = un(0, 50, !0), c.scrollX = Ht, c.scrollY = Wt, c.isDragging = c.isGesturing = c.isPressed = !1, Rt(this), c.enable = function(E) {
          return c.isEnabled || (Ve(Nt ? ie : s, "scroll", cn), a.indexOf("scroll") >= 0 && Ve(Nt ? ie : s, "scroll", dt, L, q), a.indexOf("wheel") >= 0 && Ve(s, "wheel", xt, L, q), (a.indexOf("touch") >= 0 && st || a.indexOf("pointer") >= 0) && (Ve(s, Z[0], O, L, q), Ve(ie, Z[2], zt), Ve(ie, Z[3], zt), Me && Ve(s, "click", Ne, !1, !0), ut && Ve(s, "click", Ir), ce && Ve(ie, "gesturestart", wt), l && Ve(ie, "gestureend", yt), ke && Ve(s, Ze + "enter", hr), Le && Ve(s, Ze + "leave", It), Ye && Ve(s, Ze + "move", tr)), c.isEnabled = !0, E && E.type && O(E), Zt && Zt(c)), c;
        }, c.disable = function() {
          c.isEnabled && (St.filter(function(E) {
            return E !== c && br(E.target);
          }).length || Oe(Nt ? ie : s, "scroll", cn), c.isPressed && (c._vx.reset(), c._vy.reset(), Oe(he ? s : ie, Z[1], er, !0)), Oe(Nt ? ie : s, "scroll", dt, q), Oe(s, "wheel", xt, q), Oe(s, Z[0], O, q), Oe(ie, Z[2], zt), Oe(ie, Z[3], zt), Oe(s, "click", Ne, !0), Oe(s, "click", Ir), Oe(ie, "gesturestart", wt), Oe(ie, "gestureend", yt), Oe(s, Ze + "enter", hr), Oe(s, Ze + "leave", It), Oe(s, Ze + "move", tr), c.isEnabled = c.isPressed = c.isDragging = !1, Xt && Xt(c));
     ...
     * @author: Jack Doyle, jack@greensock.com
    */
    var p, or, A, U, _t, J, Hn, Wr, Nr, sr, Gr, $r, Pe, qr, fn, ze, Wn, Nn, lr, Gn, dn, $n, Je, qn, Un, Vn, Lt, pn, hn, gn, Ur = 1, Ie = Date.now, _n = Ie(), at = 0, wr = 0, Si = function o() {
      ...
      return gt(e, "getBoundingClientRect") || ($t(e) ? function() {
      ...
      return Math.max(0, (t = "scroll" + r) && (a = gt(e, t)) ? a() - Jn(e)()[i] : $t(e) ? (_t[t] || J[t]) - (A["inner" + r] || _t["client" + r] || J["client" + r]) : e[t] - e["offset" + r]);
    }, Vr = function(e, n) {
      for (var t = 0; t < lr.length; t += 3)
        (!n || ~n.indexOf(lr[t + 1])) && e(lr[t], lr[t + 1], lr[t + 2]);
    }, vt = function(e) {
    ...
    }, ar = Math.abs, ei = "left", ti = "top", bn = "right", mn = "bottom", qt = "width", Ut = "height", Tr = "Right", kr = "Left", Cr = "Top", Er = "Bottom", te = "padding", ct = "margin", cr = "Width", wn = "Height", Se = "px", bt = function(e) {
      ...
      e._isFlipped = r, i[t.a + "Percent"] = r ? -100 : 0, i[t.a] = r ? "1px" : 0, i["border" + a + cr] = 1, i["border" + s + cr] = 0, i[t.p] = n + "px", p.set(e, i);
    }, R = [], Sn = {}, Pr, oi = function() {
      return Ie() - at > 34 && (Pr || (Pr = requestAnimationFrame(Dt)));
    }, ur = function() {
      (!Je || !Je.isPressed || Je.startX > J.clientWidth) && (T.cache++, Je ? Pr || (Pr = requestAnimationFrame(Dt)) : Dt(), at || jt("scrollStart"), at = Ie());
    }, Tn = function() {
      ...
      }), ai(hn, 1), Wr.pause(), Qt++, tt = 2, Dt(2), R.forEach(function(r) {
      ...
      if (!tt || e === 2) {
        ...
        if (nn = Cn > i ? -1 : 1, tt || (Cn = i), r && (at && !qr && t - at > 200 && (at = 0, jt("scrollEnd")), Gr = _n, _n = t), nn < 0) {
          for (ze = n; ze-- > 0; )
      ...
      Pr = 0;
    }, En = [ei, ti, mn, bn, ct + Er, ct + Tr, ct + Cr, ct + kr, "display", "flexShrink", "float", "zIndex", "gridColumnStart", "gridColumnEnd", "gridRowStart", "gridRowEnd", "gridArea", "justifySelf", "alignSelf", "placeSelf", "order"], on = En.concat([qt, Ut, "boxSizing", "max" + cr, "max" + wn, "position", ct, te, te + Cr, te + Tr, te + Er, te + kr]), Di = function(e, n, t) {
      fr(t);
      var r = e._gsap;
      if (r.spacerIsNative)
        fr(r.spacerState);
      ...
      function o(n, t) {
        or || o.register(p) || console.warn("Please gsap.registerPlugin(ScrollTrigger)"), this.init(n, t);
      }
      var e = o.prototype;
      return e.init = function(t, r) {
        ...
        if (pn(l), l._dir = C, re *= 45, l.scroller = S, l.scroll = k ? k.time.bind(k) : q, rt = q(), l.vars = t, r = r || t.animation, "refreshPriority" in t && (Gn = 1, t.refreshPriority === -9999 && (Mr = l)), Ot.tweenScroll = Ot.tweenScroll || {
        ...
        }, r && (r.vars.lazy = !1, r._initted || r.vars.immediateRender !== !1 && t.immediateRender !== !1 && r.duration() && r.render(0, !0, !0), l.animation = r.pause(), r.scrollTrigger = l, l.scrubDuration(Y), O && O.resetTo && O.resetTo("totalProgress", 0), Jt = 0, d || (d = r.vars.id)), R.push(l), z && ((!jr(z) || z.push) && (z = {
          snapTo: z
          ...
          min: 0.1,
          max: 2
        }, wt = jr(wt) ? sr(wt.min, wt.max) : sr(wt, wt), yt = p.delayedCall(z.delay || zt / 2 || 0.1, function() {
          var f = q(), b = Ie() - Bt < 500, w = Me.tween;
          if ((b || Math.abs(l.getVelocity()) < 10) && !w && !qr && ut !== f) {
            var P = (f - I) / V, me = r && !Te ? r.totalProgress() : P, H = b ? 0 : (me - er) / (Ie() - Gr) * 1e3 || 0, Q = p.utils.clamp(-P, 1 - P, ar(H / 2) * H / 0.185), Ce = P + (z.inertia === !1 ? 0 : Q), we = sr(0, 1, Dr(Ce, l)), ue = Math.round(I + we * V), K = z, ot = K.onStart, Ge = K.onInterrupt, ye = K.onComplete;
            if (f <= be && f >= I && ue !== f) {
              if (w && !w._initted && w.data <= ar(ue - f))
                return;
              z.inertia === !1 && (Q = we - P), Me(ue, {
                duration: wt(ar(Math.max(ar(Ce - me), ar(we - me)) * 0.185 / H / 0.05 || 0)),
                ease: z.ease || "power3",
                data: ar(ue - f),
                ...
                onComplete: function() {
                  l.update(), ut = q(), Jt = er = r && !Te ? r.totalProgress() : l.progress, Qe && Qe(l), ye && ye(l);
                }
          ...
          force3D: !0
        }), l.spacer = He = Xe.spacer, pr = bt(u), Ar = pr[v + C.os2], Nt = p.getProperty(u), ie = p.quickSetter(u, C.a, Se), Pn(u, He, pr), dr = sn(u)), he) {
          ...
          nt = this.markerStart = tn("start", d, Ir, C, At, Or, 0, k), kt = this.markerEnd = tn("end", d, Ir, C, At, Or, 0, k), k && (hr = p.quickSetter([nt, kt], C.a, Se)), !Le && !(B.length && gt(S, "fixedMarkers") === !0) && (Ci(ke ? J : S), p.set([Be, c], {
            force3D: !0
        ...
        l.previous = function() {
          ...
          w !== l.isReverted && (w && (xt = Math.max(q(), l.scroll.rec || 0), dt = l.progress, tr = r && r.progress()), nt && [nt, kt, Be, c].forEach(function(me) {
          ...
          if (!((Pe || !l.enabled) && !b)) {
            ...
            for (var w = Zt(), P = Xt(), me = k ? k.duration() : Yt(S, C), H = V <= 0.01, Q = 0, Ce = 0, we = t.end, ue = t.endTrigger || g, K = t.start || (t.start === 0 || !g ? 0 : u ? "0 0" : "0 100%"), ot = l.pinnedContainer = t.pinnedContainer && je(t.pinnedContainer), Ge = g && Math.max(0, R.indexOf(l)) || 0, ye = Ge, oe, De, gr, rr, fe, xe, Et, Mn, bi, Fr, Pt; ye--; )
            ...
            if (I += Q, be += Q, H && (dt = p.utils.clamp(0, 1, p.utils.normalize(I, be, xt))), l._pinPush = Ce, nt && Q && (oe = {}, oe[C.a] = "+=" + Q, ot && (oe[C.p] = "-=" + q()), p.set([nt, kt], oe)), u)
              oe = bt(u), rr = C === pe, gr = q(), it = parseFloat(Nt(C.a)) + Ce, !me && be > 1 && (Pt = (ke ? U.scrollingElement || _t : S).style, Pt = {
                style: Pt,
                value: Pt["overflow" + C.a.toUpperCase()]
              }, Pt.style["overflow" + C.a.toUpperCase()] = "scroll"), Pn(u, He, oe), dr = sn(u), De = Mt(u, !0), Mn = Le && Ft(S, rr ? Ae : pe)(), v && (Ne = [v + C.os2, V + Ce + Se], Ne.t = He, ye = v === te ? yn(u, C) + V + Ce : 0, ye && Ne.push(C.d, ye + Se), fr(Ne), ot && R.forEach(function(Lr) {
                Lr.pin === ot && Lr.vars.pinSpacing !== !1 && (Lr._subPinOffset = !0);
              }), Le && q(xt)), Le && (fe = {
                top: De.top + (rr ? gr - I : Mn) + Se,
                left: De.left + (rr ? Mn : gr - I) + Se,
                boxSizing: "border-box",
                position: "fixed"
              }, fe[qt] = fe["max" + cr] = Math.ceil(De.width) + Se, fe[Ut] = fe["max" + wn] = Math.ceil(De.height) + Se, fe[ct] = fe[ct + Cr] = fe[ct + Tr] = fe[ct + Er] = fe[ct + kr] = "0", fe[te] = oe[te], fe[te + Cr] = oe[te + Cr], fe[te + Tr] = oe[te + Tr], fe[te + Er] = oe[te + Er], fe[te + kr] = oe[te + kr], Wt = Ai(Ht, fe, Re), tt && q(0)), r ? (bi = r._initted, dn(1), r.render(r.duration(), !0, !0), We = Nt(C.a) - it + V + Ce, zr = Math.abs(V - We) > 1, Le && zr && Wt.splice(Wt.length - 2, 2), r.render(0, !0, !0), bi || r.invalidate(!0), r.parent || r.totalTime(r.totalTime()), dn(0)) : We = V, Pt && (Pt.value ? Pt.style["overflow" + C.a.toUpperCase()] = Pt.value : Pt.style.removeProperty("overflow-" + C.a));
            else if (g && q() && !k)
            ...
            }), l.start = I, l.end = be, rt = ft = tt ? xt : q(), !k && !tt && (rt < xt && q(xt), l.scroll.rec = 0), l.revert(!1, !0), yt && (ut = -1, l.isActive && q(I + V * dt), yt.restart(!0)), Pe = 0, r && Te && (r._initted || tr) && r.progress() !== tr && r.progress(tr, !0).render(r.time(), !0, !0), (H || dt !== l.progress || k) && (r && !Te && r.totalProgress(k && I < -1e-3 && !dt ? p.utils.normalize(I, be, 0) : dt, !0), l.progress = (rt - I) / V === dt ? 0 : dt), u && v && (He._pinOffset = Math.round(l.progress * We)), O && O.invalidate(), L && !tt && L(l);
          }
          ...
          if (!(k && !w && !f)) {
            ...
            if (b && (ft = rt, rt = k ? q() : P, z && (er = Jt, Jt = r && !Te ? r.totalProgress() : H)), re && !H && u && !Pe && !Ur && at && I < P + (P - ft) / (Ie() - Gr) * re && (H = 1e-4), H !== Q && l.enabled) {
              if (Ce = l.isActive = !!H && H < 1, we = !!Q && Q < 1, Ge = Ce !== we, ot = Ge || !!H != !!Q, l.direction = H > Q ? 1 : -1, l.progress = H, ot && !Pe && (ue = H && !Q ? 0 : H === 1 ? 1 : Q === 1 ? 2 : 3, Te && (K = !Ge && N[ue + 1] !== "none" && N[ue + 1] || N[ue], oe = r && (K === "complete" || K === "reset" || K in r))), mt && (Ge || oe) && (oe || Y || !r) && (Fe(mt) ? mt(l) : l.getTrailing(mt).forEach(function(fe) {
                ...
                if (f && v && (He.style[v + C.os2] = Ar), !Le)
                  ...
                  if (ye = !f && H > Q && be + 1 > P && P + 1 >= Yt(S, C), Re)
                    if (!f && (Ce || ye)) {
                      var De = Mt(u, !0), gr = P - I;
                      fi(u, J, De.top + (C === pe ? gr : 0) + Se, De.left + (C === pe ? 0 : gr) + Se);
                    } else
                      fi(u, He);
                  fr(Ce || ye ? Wt : dr), zr && H < 1 && Ce || ie(it + (H === 1 && !ye ? We : 0));
                }
              }
            ...
            if (Gt) {
              var rr = k ? P / k.duration() * (k._caScrollDist || 0) : P;
              Ct(rr + (Be._isFlipped ? 1 : 0)), Gt(rr);
            }
            hr && hr(-P / k.duration() * (k._caScrollDist || 0));
          }
        }, l.enable = function(f, b) {
          l.enabled || (l.enabled = !0, _e(S, "resize", Rr), _e(ke ? U : S, "scroll", ur), ge && _e(o, "refreshInit", ge), f !== !1 && (l.progress = dt = 0, rt = ft = ut = q()), b !== !1 && l.refresh());
          ...
          if (Ne && f) {
            ...
            Ne[b] = parseFloat(Ne[b]) + f + Se, Ne[1] = parseFloat(Ne[1]) + f + Se, fr(Ne);
          }
        ...
        return or || (p = t || Zn(), Kn() && window.document && o.enable(), or = wr), or;
        ...
        }), ve(A, "wheel", ur), ve(U, "scroll", ur), clearInterval($r), ve(U, "touchcancel", Tt), ve(J, "touchstart", Tt), Qr(ve, U, "pointerdown,touchstart,mousedown", jn), Qr(ve, U, "pointerup,touchend,mouseup", Qn), Wr.kill(), Vr(ve);
        for (var i = 0; i < T.length; i += 3)
          Kr(ve, T[i], T[i + 1]), Kr(ve, T[i], T[i + 2]);
      }, o.enable = function() {
        if (A = window, U = document, _t = U.documentElement, J = U.body, p && (Nr = p.utils.toArray, sr = p.utils.clamp, pn = p.core.context || Tt, dn = p.core.suppressOverwrites || Tt, hn = A.history.scrollRestoration || "auto", Cn = A.pageYOffset, p.core.globals("ScrollTrigger", o), J)) {
          wr = 1, Si(), ae.register(p), o.isTouch = ae.isTouch, Lt = ae.isTouch && /(iPad|iPhone|iPod|Mac)/g.test(navigator.userAgent), _e(A, "wheel", ur), Hn = [A, U, _t, J], p.matchMedia ? (o.matchMedia = function(d) {
            ...
            Kt(0, 1), jt("matchMedia");
          }), p.matchMedia("(orientation: portrait)", function() {
            return Tn(), Tn;
          })) : console.warn("Requires GSAP 3.11.0 or later"), Tn(), _e(U, "scroll", ur);
          ...
          for (i.revert || Object.defineProperty(i, "revert", {
            ...
            return Ur = 0;
          }), _e(U, "touchcancel", Tt), _e(J, "touchstart", Tt), Qr(_e, U, "pointerdown,touchstart,mousedown", jn), Qr(_e, U, "pointerup,touchend,mouseup", Qn), fn = p.utils.checkPrefix("transform"), on.push(fn), or = Ie(), Wr = p.delayedCall(0.2, Kt).pause(), lr = [U, "visibilitychange", function() {
            var d = A.innerWidth, x = A.innerHeight;
            U.hidden ? (Wn = d, Nn = x) : (Wn !== d || Nn !== x) && Rr();
          }, U, "DOMContentLoaded", Kt, A, "load", Kt, A, "resize", Rr], Vr(_e), R.forEach(function(d) {
            return d.enable(0, 1);
          }), s = 0; s < T.length; s += 3)
        ...
        var i = je(t), a = T.indexOf(i), s = $t(i);
        ...
        var a = (vt(t) ? je(t) : t).getBoundingClientRect(), s = a[i ? qt : Ut] * r || 0;
        return i ? a.right - s > 0 && a.left + s < A.innerWidth : a.bottom - s > 0 && a.top + s < A.innerHeight;
      }, o.positionInViewport = function(t, r, i) {
        vt(t) && (t = je(t));
        var a = t.getBoundingClientRect(), s = a[i ? qt : Ut], d = r == null ? s / 2 : r in Jr ? Jr[r] * s : ~r.indexOf("%") ? parseFloat(r) * s / 100 : parseFloat(r) || 0;
    ...
    D.version = "3.11.5", D.saveStyles = function(o) {
      ...
      return o ? Rr() : (or || D.register()) && Kt(!0);
    }, D.update = function(o) {
    ...
    var hi = function(e, n, t, r) {
      ...
      if (!a._isScrollT || s - a._isScrollT > 2e3) {
        for (; i && i !== J && (i.scrollHeight <= i.clientHeight && i.scrollWidth <= i.clientWidth || !(an[(d = bt(i)).overflowY] || an[d.overflowX])); )
          i = i.parentNode;
        a._isScroll = i && i !== t && !$t(i) && (an[(d = bt(i)).overflowY] || an[d.overflowX]), a._isScrollT = s;
      }
      ...
      var n = e, t = n.normalizeScrollX, r = n.momentum, i = n.allowNestedScroll, a = n.onRelease, s, d, x = je(e.target) || _t, L = p.core.globals().ScrollSmoother, Y = L && L.get(), g = Lt && (e.content && je(e.content) || Y && e.content !== !1 && !Y.smooth() && Y.content()), u = Ft(x, pe), v = Ft(x, Ae), $ = 1, re = (ae.isTouch && A.visualViewport ? A.visualViewport.scale * A.visualViewport.width : A.outerWidth) / A.innerWidth, W = 0, Qe = Fe(r) ? function() {
        ...
        d = Yt(x, pe), Ke = sr(Lt ? 1 : 0, d), t && (k = sr(0, Yt(x, Ae))), ee = Qt;
      }, C = function() {
      ...
      return g && p.set(g, {
        ...
        z = !1;
        var N = $;
        $ = yr((A.visualViewport && A.visualViewport.scale || 1) / re), S.pause(), N !== $ && Rn(x, $ > 1.01 ? !0 : t ? !1 : "x"), Ot = v(), ke = u(), mt(), ee = Qt;
        ...
        if (Qt !== ee && mt(), he && t && v(k(l[2] === he ? Ot + (N.startX - N.x) : v() + he - l[1])), ce) {
          u.offset && C();
          var Zt = ge[2] === ce, Xt = Zt ? ke + N.startY - N.y : u() + ce - ge[1], ut = Ke(Xt);
          Zt && Xt !== ut && (ke += ut - Xt), u(ut);
        }
        (ce || he) && Dt();
      }, e.onEnable = function() {
    ...
    D.sort = function(o) {
      ...
      return Je && Je.target === e.target && Je.kill(), $t(e.target) && (Je = e), e;
    }, D.core = {
      _getVelocityProp: un,
      _inputObserver: gi,
      _scrollers: T,
...
class xi {
  constructor(h, _ = {}) {
    ...
    this.speed = _.speed ?? 1, this.velocity = _.velocity ?? 0, this.direction = _.direction || "rtl", this.onUpdate = _.onUpdate, this.onCreated = _.onCreated, this.target = h instanceof Function ? h() : h, this.motion = new _r(() => {
      var Ue;
      let G = null;
      ...
      const Ze = $e.context(() => {
        ...
        const St = (this.scrollTrigger.getVelocity() ?? 0) * this.velocity * Rt, T = $e.getProperty(j, "x"), Ee = this.speed * $e.ticker.deltaRatio() * -ht - St, lt = $e.utils.wrap(0, -F.width, T + Ee), ir = $e.utils.normalize(0, -F.width, lt);
        pt(lt), (gt = this.onUpdate) == null || gt.call(this, ir);
      });
      return (Ue = this.onCreated) == null || Ue.call(this), () => {
        var Z;
        for (Ze.kill(!0), $e.ticker.remove(nr); qe.length; )
